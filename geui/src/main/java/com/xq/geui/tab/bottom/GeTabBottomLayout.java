...
import com.xq.geui.tab.common.IGeTabLayout;
import com.xq.log.HiDisplayUtil;
import com.xq.log.HiViewUtil;
 ...
 * 2. 中间高度超过，凸起布局
 */
public class GeTabBottomLayout  extends FrameLayout implements IGeTabLayout<GeTabBottom,GeTabBottomInfo<?>> {

    private static final String TAG_TAB_BOTTOM = "TAG_TAB_BOTTOM";

    private List<OnTabSelectedListener<GeTabBottomInfo<?>>> tabSelectedChangeListeners = new ArrayList<>();
    private GeTabBottomInfo<?> selectedInfo;
    ...
    private List<GeTabBottomInfo<?>> infoList;//数据

    public GeTabBottomLayout(@NonNull Context context) {
        super(context);
    }

    public GeTabBottomLayout(@NonNull Context context,@Nullable AttributeSet attrs) {
        super(context,attrs);
    }
    public GeTabBottomLayout(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
    }

    @Override
    public void inflateInfo(@NonNull List<GeTabBottomInfo<?>> infoList) {
        if(infoList.isEmpty()){
            return;
        }
        ...
        Iterator<OnTabSelectedListener<GeTabBottomInfo<?>>> iterator = tabSelectedChangeListeners.iterator();
        while (iterator.hasNext()) {
            if (iterator.next() instanceof GeTabBottom) {
                iterator.remove();
            }
        }
        ...
        for (int i = 0; i < infoList.size(); i++) {
            final GeTabBottomInfo<?> info = infoList.get(i);
            //Tips：为何不用LinearLayout：当动态改变child大小后Gravity.BOTTOM会失效
            LayoutParams params = new LayoutParams(width, height);
            params.gravity = Gravity.BOTTOM;
            params.leftMargin = i * width;

            GeTabBottom tabBottom = new GeTabBottom(getContext());
            tabSelectedChangeListeners.add(tabBottom);
            tabBottom.setGeTabInfo(info);
    ...
    @Override
    public void addTabSelectedChangeListener(OnTabSelectedListener<GeTabBottomInfo<?>> listener) {
    ...
    public static void setTabHeight(float tabBottomHeight) {
        GeTabBottomLayout.tabBottomHeight = tabBottomHeight;
    }
    ...
    @Override
    public GeTabBottom findTab(@NonNull  GeTabBottomInfo<?> info) {
        ViewGroup ll = findViewWithTag(TAG_TAB_BOTTOM);
        for (int i = 0; i < ll.getChildCount(); i++) {
            View child = ll.getChildAt(i);
            if (child instanceof GeTabBottom) {
                GeTabBottom tab = (GeTabBottom) child;
    ...
    @Override
    public void defaultSelected(@NonNull GeTabBottomInfo<?> defaultInfo) {
        onSelected(defaultInfo);
    }

    private void onSelected(@NonNull GeTabBottomInfo<?> nextInfo) {
        for(OnTabSelectedListener<GeTabBottomInfo<?>> listener : tabSelectedChangeListeners) {
    ...
    private void addBackground() {
        View view = LayoutInflater.from(getContext()).inflate(R.layout.ge_bottom_layout_bg, null);
